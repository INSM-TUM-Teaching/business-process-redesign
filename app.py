...
def delete_activity_endpoint():
    ...
    try:
        ...
        global current_matrix
        if current_matrix is None:
            return jsonify({"success": False, "error": "No matrix available. Please analyze traces first."})
        
        # Check if activity exists
        if activity not in current_matrix.activities:
            return jsonify({"success": False, "error": f"Activity '{activity}' not found in matrix"})
