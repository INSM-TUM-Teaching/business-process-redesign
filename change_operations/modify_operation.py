...
def modify_dependencies(
    ...
    for (from_act, to_act), (temporal_dep, existential_dep) in list(modified_deps.items()):
        if temporal_dep is not None and temporal_dep.type == TemporalType.DIRECT:
    ...
    for from_act, to_act, temporal_dep, existential_dep in modifications:
        ...
        if (from_act, to_act) in modified_deps:
            ...
            new_temporal = TemporalDependency(temporal_dep.type, direction=temporal_dep.direction) if temporal_dep is not None else None
            new_existential = ExistentialDependency(existential_dep.type, direction=existential_dep.direction) if existential_dep is not None else None
...
# Backward compatibility wrapper for hardcoded UI demos
def modify_dependency(
    ...
    """
    Modify a single dependency (backward compatibility wrapper).

    This is a compatibility wrapper that calls modify_dependencies() with a single modification.
    Used by hardcoded UI demos.

    Args:
        matrix: The input adjacency matrix
        from_activity: The name of the activity from which the dependency is seen
        to_activity: The name of the activity to which the dependency is seen
        temporal_dep: Temporal dependency type which should become the new one
        existential_dep: Existential dependency type which should become the new one
        temporal_direction: Direction for temporal dependency (defaults to FORWARD)
        existential_direction: Direction for existential dependency (defaults to FORWARD)
    ...
    """
    # Get existing dependency to preserve direction if not specified
    existing_deps = matrix.get_dependencies()
    existing_temp, existing_exist = existing_deps.get((from_activity, to_activity), (None, None))

    # Determine directions
    if temporal_dep is not None:
        if temporal_direction is None:
            temp_dir = existing_temp.direction if existing_temp is not None else Direction.FORWARD
        else:
            temp_dir = temporal_direction
        temp_dependency = TemporalDependency(temporal_dep, direction=temp_dir)
    else:
        # If no temporal dep specified, preserve existing or use INDEPENDENCE
        if existing_temp is not None:
            temp_dependency = existing_temp
        else:
            temp_dependency = TemporalDependency(TemporalType.INDEPENDENCE, direction=Direction.BOTH)

    if existential_dep is not None:
        if existential_direction is None:
            exist_dir = existing_exist.direction if existing_exist is not None else Direction.FORWARD
        else:
            exist_dir = existential_direction
        exist_dependency = ExistentialDependency(existential_dep, direction=exist_dir)
    else:
        # If no existential dep specified, preserve existing or use INDEPENDENCE
        if existing_exist is not None:
            exist_dependency = existing_exist
        else:
            exist_dependency = ExistentialDependency(ExistentialType.INDEPENDENCE, direction=Direction.BOTH)

    # Call the new modify_dependencies function with a single modification
    modifications = [(from_activity, to_activity, temp_dependency, exist_dependency)]
    modified_matrix, _ = modify_dependencies(matrix, modifications)
