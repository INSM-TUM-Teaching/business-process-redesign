...
def test_satisfies_existential_implication_ok(sample_activities):
    deps = {
        ("A", "B"): ExistentialDependency(
            ExistentialType.IMPLICATION, Direction.FORWARD
        )
    }
    assert (
        satisfies_existential_constraints({"A", "B"}, sample_activities, deps) == True
    )
    ...
    assert (
        satisfies_existential_constraints({"A", "C"}, sample_activities, deps) == False
    )
...
def test_satisfies_existential_equivalence_ok(sample_activities):
    deps = {
        ("A", "B"): ExistentialDependency(ExistentialType.EQUIVALENCE, Direction.BOTH)
    }
    assert (
        satisfies_existential_constraints({"A", "B"}, sample_activities, deps) == True
    )
...
def test_satisfies_existential_or_fail(sample_activities):
    ...
    assert (
        satisfies_existential_constraints({"C"}, sample_activities, deps) == False
    )  # A and B not in subset
...
def test_satisfies_existential_nand_fail(sample_activities):
    deps = {("A", "B"): ExistentialDependency(ExistentialType.NAND, Direction.BOTH)}
    assert (
        satisfies_existential_constraints({"A", "B"}, sample_activities, deps) == False
    )
...
def test_satisfies_existential_independence(sample_activities):
    deps = {
        ("A", "B"): ExistentialDependency(ExistentialType.INDEPENDENCE, Direction.BOTH)
    }
    assert (
        satisfies_existential_constraints({"A", "B"}, sample_activities, deps) == True
    )
...
def test_generate_acceptance_variants_choice_or():
    adj = AdjacencyMatrix(activities=["A", "B"])
    adj.add_dependency(
        "A", "B", None, ExistentialDependency(ExistentialType.OR, Direction.BOTH)
    )
    adj.add_dependency(
        "B", "A", None, ExistentialDependency(ExistentialType.OR, Direction.BOTH)
    )

    variants = generate_acceptance_variants(adj)
    expected = sorted([["A"], ["B"], ["A", "B"], ["B", "A"]])
    assert sorted([sorted(s) for s in variants]) == sorted(
        [sorted(s) for s in expected]
    )
...
def test_generate_acceptance_variants_from_yaml_first_prototype():
    ...
    for variant in variants:
        ...
        if "C" in variant and "D" in variant:
            pytest.fail(
                f"Variant {variant} contains both C and D, which violates C|D NAND constraint."
            )

    assert found_ABCE, "Expected variant [A,B,C,E] not found."

    adj_indep = AdjacencyMatrix(activities=["X", "Y"])
    adj_indep.add_dependency(
        "X",
        "Y",
        TemporalDependency(TemporalType.INDEPENDENCE, Direction.BOTH),
    ...
    assert actual_indep == expected_indep, (
        f"Expected {expected_indep}, got {actual_indep}"
    )
