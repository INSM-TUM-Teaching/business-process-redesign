...
def test_infer_existential_equivalence():
    traces = [["A", "B"], ["C"]]  # A <=> B
    expected = (ExistentialDependency(type=ExistentialType.EQUIVALENCE), "A", "B")
...
def test_infer_existential_implication():
    ...
    expected_ab = (ExistentialDependency(type=ExistentialType.IMPLICATION), "A", "B")
    assert (
        infer_existential_dependency("A", "B", traces_a_implies_b, 1.0) == expected_ab
    )
    assert (
        infer_existential_dependency("B", "A", traces_a_implies_b, 1.0) == expected_ab
    )
...
def test_traces_to_adj_matrix_simple_variant():
    ...
    assert dep_ab_temp == TemporalDependency(TemporalType.DIRECT)
    assert dep_ab_exist == ExistentialDependency(ExistentialType.EQUIVALENCE)

    # B->C
    dep_bc_temp, dep_bc_exist = adj_matrix.get_dependency("B", "C")
    assert dep_bc_temp == TemporalDependency(TemporalType.DIRECT)
    assert dep_bc_exist == ExistentialDependency(ExistentialType.EQUIVALENCE)

    # A->C
    dep_ac_temp, dep_ac_exist = adj_matrix.get_dependency("A", "C")
    assert dep_ac_temp == TemporalDependency(TemporalType.EVENTUAL)
    assert dep_ac_exist == ExistentialDependency(ExistentialType.EQUIVALENCE)
    ...
    assert dep_ba_temp is None  # B does not precede A in this trace
    assert dep_ba_exist == ExistentialDependency(ExistentialType.EQUIVALENCE)
...
def test_traces_to_adj_matrix_choice():
    ...
    assert dep_ab_t == TemporalDependency(TemporalType.DIRECT)
    assert dep_ab_e is None

    dep_ba_t, dep_ba_e = adj_matrix.get_dependency("B", "A")
    assert dep_ba_t is None
    assert dep_ba_e == ExistentialDependency(ExistentialType.IMPLICATION)

    dep_ac_t, dep_ac_e = adj_matrix.get_dependency("A", "C")
    assert dep_ac_t == TemporalDependency(TemporalType.DIRECT)
    assert dep_ac_e is None

    dep_ca_t, dep_ca_e = adj_matrix.get_dependency("C", "A")
    assert dep_ca_t is None
    assert dep_ca_e == ExistentialDependency(ExistentialType.IMPLICATION)

    dep_bc_t, dep_bc_e = adj_matrix.get_dependency("B", "C")
    assert dep_bc_t is None
    assert dep_bc_e == ExistentialDependency(ExistentialType.NEGATED_EQUIVALENCE)

    dep_cb_t, dep_cb_e = adj_matrix.get_dependency("C", "B")
    assert dep_cb_t is None
    assert dep_cb_e == ExistentialDependency(ExistentialType.NEGATED_EQUIVALENCE)